...
textinfo = RNFactory.createText("Touch button1 to stop/start timer", { size = 18, top = 10, left = 5, width = 300, height = 50 })
...
function button1UP(event)
    if counting then
        RNMainThread.suspendAction(actionId)
        counting = false
    else
        RNMainThread.resumeAction(actionId)
        counting = true
    end
end
