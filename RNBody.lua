...
self.body:destroy()
self.sprite:setAlpha(1) --qui andrebbe distrutta la RNSprite ma per ora non si puo
--per distruggere la sprite mi serve sapere di che RNScene è messa la RNSprite cosi
--rimuovo la prop dal layer
--rimuove RNPhysics dalla body list e scalo gli index di tutti gli altri RNBody nella lista manualmente
len=table.getn(self.parentList)
ind=self.indexinlist
for i=1,len,1 do
if (i==ind) then 
for k=ind, len,1 do self.parentList[k-1]=self.parentList[k] self.parentList[k].indexinlist=k-1 end
self.parentList[len]=nil
end
end
--pero manualmente devo aggiornare gli indexinlist degli altri
...
else self.body:setMassData(mass,I,centerX,centerY) end
end
...
else self.body:setTransform(positionX,positionY,angle) end
end
...
else self.body:applyForce(forceX,forceY,pointX,pointY) end
end
...
else self.body:applyLinearImpulse(impulseX,impulseX,pointX,pointY) end
end
